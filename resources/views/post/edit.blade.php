  ...
  <main class="col-9 pe-2">
    <section class="col-12 bg-primary-grey card border-0 h-full">
      ...
      <div class="card-body col-12">
        <form method="post" action="/post/{{ $post->id }}" enctype="multipart/form-data" class="d-flex flex-wrap gap-3 col-12">
          @method('put')
          @csrf
          <div class="gap-halfed-width">
            @if($post->image)
              <img src="{{ asset('storage/' . $post->image) }}" class="img-preview col-12 mb-3 h-fit">
            @else
              <img class="img-preview col-12 mb-3 h-fit">
            @endif
            <div class="mb-3">
